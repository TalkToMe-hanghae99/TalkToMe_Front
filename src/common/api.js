...
export const apis = {
  //구글 로그인
  googlelogin: (user) => instance.get(`/user/google`, user),

  // 댓글 조회
  getComment: (boardId) => instance.get(`/comment/${boardId}`),

  //댓글 추가
  addComment: (comment) =>
    instance.post(`/comment/${comment.boardId}`, comment),
  ...
  getList: (param, page, size, sortBy, isAsc) =>
    instance.get(
      `/api/main/${param}?page=${page}&size=${size}&sortBy=${sortBy}&isAsc=${isAsc}`
    ),
  // 찜하기 기능(Post)
  ...
  addCoupon: (coupon_content) =>
    instance.post("api/admin/coupon", coupon_content),
  delCoupon: (coupon_id) => instance.delete(`/api/admin/coupon/${coupon_id}`),
  editCoupon: (coupon_id, coupon_content) =>
    instance.put(`api/admin/coupon/update/${coupon_id}`, coupon_content),
